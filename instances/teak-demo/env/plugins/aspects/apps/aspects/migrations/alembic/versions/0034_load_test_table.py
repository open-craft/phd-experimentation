...
def upgrade():
    op.execute(
        f"""
        CREATE TABLE IF NOT EXISTS event_sink.load_test_runs
        {on_cluster}
        (
            run_id     String,
    ...
    op.execute(
        f"""
        CREATE TABLE IF NOT EXISTS event_sink.load_test_stats
        {on_cluster}
        (
            run_id    String,
            timestamp DateTime default now(),
...
def downgrade():
    op.execute(
        "DROP TABLE IF EXISTS event_sink.load_test_stats"
        f"{on_cluster}"
    )

    op.execute(
        "DROP TABLE IF EXISTS event_sink.load_test_runs"
        f"{on_cluster}"
    )