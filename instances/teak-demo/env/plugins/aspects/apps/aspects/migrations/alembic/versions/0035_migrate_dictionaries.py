...
def upgrade():
    op.execute(
        f"""
        DROP DICTIONARY IF EXISTS event_sink.user_pii
        {on_cluster}
        """
    )
    op.execute(
    ...
    op.execute(
        f"""
        DROP DICTIONARY IF EXISTS event_sink.course_block_names
        {on_cluster}
        """
    )
    op.execute(
    ...
    op.execute(
        f"""
        DROP DICTIONARY IF EXISTS event_sink.course_names
        {on_cluster}
        """
    )
    op.execute(
        f"""
        DROP DICTIONARY IF EXISTS event_sink.course_names_dict
        {on_cluster}
        """
    )
...
def downgrade():
    ...
    op.execute(
        f"""
        CREATE DICTIONARY event_sink.course_names_dict
        {on_cluster}
        (
            course_key String,
            course_name String,
    ...
    op.execute(
        f"""
        CREATE TABLE event_sink.course_names
        {on_cluster}
        (
            course_key String,
            course_name String,
            course_run String,
            org String
        ) engine = Dictionary(event_sink.course_names_dict);
        """
    )
    ...
    op.execute(
        f"""
        CREATE TABLE IF NOT EXISTS event_sink.most_recent_course_blocks
        {on_cluster}
        (
    ...
    op.execute(
        f"""
        create materialized view if not exists event_sink.most_recent_course_blocks_mv
        {on_cluster}
        to event_sink.most_recent_course_blocks as
        select
            location,
            display_name,
    ...
    op.execute(
        """
        insert into event_sink.most_recent_course_blocks (
    ...
    op.execute(
        ...
        SOURCE(CLICKHOUSE(
            ...
            query "
                ...
                from event_sink.most_recent_course_blocks
                final
            "
        ))
