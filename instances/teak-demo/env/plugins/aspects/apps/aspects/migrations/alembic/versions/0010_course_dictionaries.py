...
def upgrade():
    ...
    op.execute(f"""
        DETACH DICTIONARY IF EXISTS event_sink.course_names
        {on_cluster}
    """)
    op.execute(f"""
        DROP TABLE IF EXISTS event_sink.course_names 
        {on_cluster}
        SYNC
    """)
    op.execute(f"""
    ...
    """)
    op.execute(f"""
        DETACH DICTIONARY IF EXISTS event_sink.course_block_names 
        {on_cluster}
    """)
    op.execute(f"""
    ...
    op.execute(f"""
        ...
        event_sink.course_block_names_dict
        {on_cluster}
        SYNC
    """)
    op.execute(
    ...
    op.execute(
        f"""
        CREATE OR REPLACE TABLE event_sink.course_names
        {on_cluster}
        (
            course_key String,
            course_name String
