...
def upgrade():
    ...
    op.execute(
        f"""
        CREATE TABLE IF NOT EXISTS event_sink.course_relationships
        {on_cluster}
        (
            course_key String NOT NULL,
...
def downgrade():
    op.execute(
        "DROP TABLE IF EXISTS event_sink.course_relationships"
        f"{on_cluster}"
    )
    op.execute(
        "DROP TABLE IF EXISTS event_sink.course_blocks"
        f"{on_cluster}"
    )
    op.execute(
        "DROP TABLE IF EXISTS event_sink.course_overviews"
        f"{on_cluster}"
    )