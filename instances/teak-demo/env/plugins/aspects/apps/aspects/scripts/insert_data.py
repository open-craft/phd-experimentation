...
def sink_files():
    ...
    client.query(
    f"""
    INSERT INTO event_sink.aspects_data FORMAT JSONEachRow {json.dumps(files)}
    """
    )

def load_files():
    result = client.query("SELECT path, content from event_sink.aspects_data OPTIMIZE FINAL")
