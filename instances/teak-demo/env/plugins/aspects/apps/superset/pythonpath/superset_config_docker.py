...
OPENEDX_API_URLS = {
    "get_courses": urljoin(OPENEDX_LMS_ROOT_URL, "/api/courses/v1/courses/?permissions={permission}&username={username}"),
    "get_preference": urljoin(OPENEDX_LMS_ROOT_URL, "api/user/v1/preferences/{username}"),
}
...
LANGUAGES = {'en': {'flag': 'us', 'name': 'English'}, 'es': {'flag': 'es', 'name': 'Español'}, 'it': {'flag': 'it', 'name': 'Italian'}, 'fr': {'flag': 'fr', 'name': 'French'}, 'zh': {'flag': 'cn', 'name': 'Chinese'}, 'ja': {'flag': 'jp', 'name': 'Japanese'}, 'de': {'flag': 'de', 'name': 'German'}, 'pt': {'flag': 'pt', 'name': 'Portuguese'}, 'ru': {'flag': 'ru', 'name': 'Russian'}, 'ko': {'flag': 'kr', 'name': 'Korean'}, 'sk': {'flag': 'sk', 'name': 'Slovak'}, 'sl': {'flag': 'si', 'name': 'Slovenian'}, 'nl': {'flag': 'nl', 'name': 'Dutch'}}

# This is a list of Open edX supported locales, some of which are not supported
# by Superset at this time.
DASHBOARD_LOCALES = ['ar', 'da', 'de_DE', 'el', 'en', 'es_419', 'es_ES', 'fr_CA', 'he', 'hi', 'id', 'it_IT', 'pt_BR', 'pt_PT', 'ru', 'th', 'tr_TR', 'uk', 'zh_CN']
...
AUTH_ROLES_MAPPING = {
    ...
    "student": ["Student"], # Course students
    "operator": ["Operator"], # Installation operators
    "public": ["Public"],    # AKA anonymous users
}

for locale in DASHBOARD_LOCALES:
    AUTH_ROLES_MAPPING[f"instructor-{locale}"] = [f"Instructor - {locale}"]
    AUTH_ROLES_MAPPING[f"student-{locale}"] = [f"Student - {locale}"]
    AUTH_ROLES_MAPPING[f"operator-{locale}"] = [f"Operator - {locale}"]
